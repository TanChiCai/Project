...
class GPA_Calculator:
    ...
    def __init__(self):
        ...
        self.container = tk.Frame(self.window)  # create a frame for hold up button,label which is attached to the main window self.window.
        ...
        self.window.iconbitmap('C:/Users/user/OneDrive/Desktop/python/python_Assignment/DCS1S2G2-BehKokWeiLohKaiTerngTanChiCai/GPA.ico')

        self.current_page = "main_page"  # Track the current page

        # Create a canvas to hold the scrollable content
        self.canvas = tk.Canvas(self.window)
        self.canvas.grid(row=0, column=0, sticky="nsew")  # Grid canvas in the main window

        # Create a vertical scrollbar linked to the canvas
        self.v_scrollbar = ttk.Scrollbar(self.window, orient="vertical", command=self.canvas.yview)
        self.v_scrollbar.grid(row=0, column=1, sticky="ns")  # Position scrollbar on the right

        # Create a horizontal scrollbar linked to the canvas
        self.h_scrollbar = ttk.Scrollbar(self.window, orient="horizontal", command=self.canvas.xview)
        self.h_scrollbar.grid(row=1, column=0, sticky="ew")  # Position scrollbar at the bottom

        # Configure the canvas to use the scrollbars
        self.canvas.configure(yscrollcommand=self.v_scrollbar.set, xscrollcommand=self.h_scrollbar.set)

        # Create a frame that will contain all your content
        self.content_frame = tk.Frame(self.canvas)

        # Create a window on the canvas that contains the content frame
        self.canvas.create_window((0, 0), window=self.content_frame, anchor="nw")

        # Configure the frame's size so that the scrollbars will appear when needed
    ...
    def on_first_run(self):
        ...
        if not self.i:  # make sure only run one times only for beginning
            ...
            if self.focused_widget:  # make sure only press enter key directly without doing any process will prompt error
                ...
                self.window.bind('<Return>',lambda event: self.on_first_run())  # make sure this function  will run again if press
    ...
    def create_main_page(self):  # create main page
        ...
        # Add More Course button (centered horizontally)
        self.add_button = tk.Button(self.main_page, text="+ Add More Course", command=self.add_coursebutton,
                                    font=("Arial", 16))
        self.add_button.grid(row=self.rows + 3, column=1, pady=10)

        # Back button (centered horizontally, placed below Add More Course)
        self.Back_to_main_menu = tk.Button(self.main_page, text="Back", command=self.Back_to_main_menu,
                                           font=("Arial", 16))
        self.Back_to_main_menu.grid(row=self.rows + 4, column=0, pady=10)

        # Calculate button (centered horizontally, placed below Back button)
        self.calculate_button = tk.Button(self.main_page, text="Calculate", command=self.calculatebutton,
                                          font=("Arial", 16))
        self.calculate_button.grid(row=self.rows + 4, column=1, pady=10)

        # Clear button (next to Calculate button, placed on the same row)
        self.clear_button = tk.Button(self.main_page, text="Clear", command=self.clearbutton, font=("Arial", 16))
        self.clear_button.grid(row=self.rows + 4, column=2, pady=10)

        # Ensure equal distribution of space across columns for better right positioning
        self.main_page.grid_columnconfigure(0, weight=1)
        self.main_page.grid_columnconfigure(1, weight=1)
        self.main_page.grid_columnconfigure(2, weight=1)
    ...
    def create_table_row(self):
        ...
        for j in range(columns):
            ...
            if j == 0:
                ...
                self.dropdown = ttk.Combobox(self.main_page, textvariable=entry_var, values=self.Course, width=50,
                                             font=("Arial", 16))
                ...
                self.entry = tk.Entry(self.main_page, textvariable=entry_var, width=35, justify="center",
                                      relief="solid",
                                      font=("Arial", 16))
    ...
    def display_results(self, event=None):
        ...
        tk.Label(self.display_result_page, text=semester, font=("Arial", 12)).grid(row=1, column=0, columnspan=3,
                                                                                   pady=5)
        tk.Label(self.display_result_page, text=current_date, font=("Arial", 10)).grid(row=2, column=0, columnspan=3,
                                                                                       pady=5)

        # GPA and CGPA section
        gpa_frame = tk.Frame(self.display_result_page)
        gpa_frame.grid(row=3, column=0, columnspan=3, pady=10)
        ...
        tree_frame = tk.Frame(self.display_result_page)
        ...
        # Configure grid weights for dynamic resizing
        self.display_result_page.grid_rowconfigure(1, weight=0)  # Allow Treeview to expand vertically
        self.display_result_page.grid_rowconfigure(2, weight=0)  # Horizontal scrollbar row
        self.display_result_page.grid_columnconfigure(0, weight=1)  # Main content column (Treeview)
        self.display_result_page.grid_columnconfigure(0, weight=1)  # Allow Treeview to expand horizontally
        ...
        # Configure the tag for "F" grade rows: light pink background and red foreground
        self.tree.tag_configure("light red", foreground="red")
        ...
        # Termination session
        tk.Label(self.display_result_page, text=terminatingsession, font=("Arial", 12), fg="red").grid(row=5, column=0,
                                                                                                       columnspan=3,
                                                                                                       pady=10)
        button_frame=tk.Frame(self.display_result_page)     #remember write grid
        button_frame.grid(row=6, column=0, columnspan=2, sticky="nsew", padx=10, pady=5)

        # Buttons Section (aligned with the Treeview columns)
        self.Back = tk.Button(button_frame, text="Back", command=lambda: self.back("Calculation"),
                              font=("Arial", 16))
        self.Save = tk.Button(button_frame, text="Save", command=lambda: self.save(event=None),
                              font=("Arial", 16))

        # Grid the buttons below the table
        self.Back.grid(row=0, column=0, columnspan=2, pady=5, padx=625)  # Right button
        self.Save.grid(row=0, column=0, columnspan=4, pady=5, padx=825)  # Left button
    ...
    def save(self, event):
        ...
        # Step 1: Temporarily maximize the window
        self.window.state("zoomed")     #maximize
        self.window.update()  # Ensure the maximized state is fully applied,  updates both idle tasks and processes all the events in the event loop (such as mouse clicks, key presses, etc.)

        # The UI updates and the button can be clicked again during the task (if you set some additional interaction or more events in your code). You'll see that you can still interact with the UI while the task is running.
        # Wait briefly to ensure the window is fully rendered
        time.sleep(1.2)

        # Step 2: Get coordinates of the maximized window
        x = self.window.winfo_rootx()   #x-coordinate of the top-left corner of the window relative to the screen's coordinate system (i.e., the entire screen or display, not just the window's content area).
        y = self.window.winfo_rooty()   #y-coordinate of the top-left corner of the window relative to the screen's coordinate system.

        # Use fixed dimensions (1920x578 px)
        width = 1920
        height = 578


        # Capture the specific area  bbox(bounding box)
        screenshot = ImageGrab.grab(bbox=(x, y, x + width, y + height))

        # Step 3: Open file dialog to save the screenshot
        ...
        # Step 4: Restore the window to default "normal" state
        self.window.state("normal")     #go back to default/standard
        self.window.update_idletasks()  # Ensure updates are applied quickly,   only the idle tasks (like layout updates, geometry changes) without processing events like user input(don't need to process events like mouse clicks or keyboard input right away).

        #Only the UI layout will be updated (i.e., the label changes to reflect progress), but user input is not handled. You will notice that you can't click the other button during the task because update_idletasks() doesn’t process events like button presses.
